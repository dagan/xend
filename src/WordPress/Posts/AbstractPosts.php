...
abstract class AbstractPosts implements PostsInterface {
    ...
    public function getAuthor($post) {
        ...
        if (!$post instanceof \Xend\WordPress\Posts\Post) {
            throw new Exception("The first argument passed to getAuthor() must be an integer or Post object");
        }

        $wp_user = get_user_by("id", $post->authorId);
        return ($wp_user instanceof \WP_User) ? new \Xend\WordPress\Users\User($wp_user) : false;
    }
    ...
    public function getFilteredAuthorName(Post $post) {
